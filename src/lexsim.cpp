...
double lexsimexact_t::get_sim(string ref, string hyp, int mode){
  if (mode == yisi::INP_MODE){
    cerr<<"ERROR: exact matching lex sim model is not defined in crosslingual settings. Exiting..."<<endl;
    exit(1);
  }
  if( ref == hyp){
    return 1.0;
...
double lexsimlcs_t::get_sim(string ref, string hyp, int mode){
  if (mode == yisi::INP_MODE){
    cerr<<"ERROR: longest common subsequence lex sim model is not defined in crosslingual settings. Exiting..."<<endl;
    exit(1);
  }
  double lcs_n=0.0;
  ...
  //cerr<<"lcs of ("<<ref<<" , "<<hyp<<") = (" << lcs_n<<","<<ref_n<<","<<hyp_n<<")" <<endl;
...
vector<double>& lexsimw2v_t::get_wv(string word, int mode){
  if (mode == yisi::INP_MODE){
    cerr<<"ERROR: w2v lex sim model is not defined in crosslingual settings. Exiting..."<<endl;
    exit(1);
  }
...
void lexsimw2v_t::write_txtw2v(std::string path){
  ...
  if (!os){
    cerr <<"ERROR: fail to open file to write w2v model. No model written."<<endl;
  } else {
...
double lexsimemapw2v_t::get_sim(string ref, string hyp){
  ...
  if (emap_m.find(ref) != emap_m.end()){
	...
	result += emap_m[ref][i].second * simfunc(func_m, this->get_wv(emap_m[ref][i].first), this->get_wv(hyp));
      }
...
double lexsimbiw2v_t::get_sim(string s1, string hyp, int mode){
  if ((lowercase(s1)==lowercase(hyp)) && 
      (((mode == yisi::INP_MODE) && ((inpembeddings_m.find(s1)==inpembeddings_m.end()) || outembeddings_m.find(hyp)==outembeddings_m.end())) || 
       (mode != yisi::INP_MODE))){
    return 1.0;
  } else {
...
lexsim_t::lexsim_t(string name, string out_path, string inp_path){
  if (name == "exact"){
    ...
    cerr<<"ERROR: unknown lexsim model name " <<name<<endl;
  }
  lexsim_name_m = name;
...
lexsim_t::lexsim_t(lexsim_t& rhs){
  if (rhs.lexsim_name_m == "exact"){
    ...
    //lexsim_p = new lexsimemapw2v_t(rhs.lexsim_path_m, rhs.lexsim_func_m, rhs.lexsim_addpath_m, rhs.lexsim_addfunc_m);
...
void yisi::read_binw2v(string path, map<string, vector<double> >& model, int& dimension){
  ...
  if (!W2V){
    cerr <<"Fail to open w2v model. Exiting..."<<endl;
    exit(1);
  }
...
double yisi::simfunc(string funcname, vector<double>& ref, vector<double>& hyp){
  if (funcname == "ucosine"){
    ...
    cerr<<"ERROR: unknown lexsim func " <<funcname<<endl;
    exit(1);
  }
}
