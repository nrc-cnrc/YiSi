...
double lexweightmodel_t::get_weight(string lex){
  if (lexweight_m.find(lex) != lexweight_m.end()){
    return lexweight_m[lex];
  } else {
    string l = lowercase(lex);
    if (lexweight_m.find(l) != lexweight_m.end()){
      return lexweight_m[l];
    } else {
      return eps_m;
    }
  }
...
lexweightfile_t::lexweightfile_t(string path){
  ...
  if (!WT){
    cerr <<"ERROR: fail to open weight table file. Exiting..."<<endl;
    exit(1);
  }
  while (!WT.eof()){
    string lex;
...
lexweightlearn_t::lexweightlearn_t(string path){
  cerr <<"Learning lex weight from "<<path<<" ... ";
  ifstream C(path.c_str());
...
void lexweightlearn_t::learn(vector<vector<string> > tokens){
  ...
  for (auto it=tokens.begin(); it != tokens.end(); it++){
    ...
    for (auto jt=sent.begin(); jt!=sent.end(); jt++){
      if (lexweight_m.find(jt->first) != lexweight_m.end()){
        lexweight_m[jt->first] += 1.0;
      } else {
        lexweight_m[jt->first] = 1.0;
      }
    }
  ...
  for (auto it=lexweight_m.begin(); it!=lexweight_m.end(); it++){
    auto c = it->second;
    it->second = log2(1 + ((N + 1.0)/(c + 1.0)));
  }
  eps_m = log2(1+((N + 1.0)/(1.0)));
}
...
lexweight_t::lexweight_t(string name, string path){
  if (name == "uniform"){
    ...
    cerr<<"ERROR: unknown lexweight model name " <<name<<endl;
  }
