...
namespace yisi {
  ...
  public:
    ...
    phrasesim_t(opt_T opt){
      ...
      //std::cerr<<"reflw_name\t"<<opt.reflexweight_name_m<<"\treflw_path\t"<<opt.reflexweight_path_m<<"\treflw_threshold\t"<<opt.reflexweight_threshold_m<<std::endl;
      reflexweight_p = new lexweight_t(opt.reflexweight_name_m, opt.reflexweight_path_m);
      if (opt.hyplexweight_name_m != ""){
	//std::cerr<<"hyplw_name\t"<<opt.hyplexweight_name_m<<"\thyplw_path\t"<<opt.hyplexweight_path_m<<"\thyplw_threshold\t"<<opt.hyplexweight_threshold_m<<std::endl;
    ...
    std::pair<double, double> operator()(std::vector<std::string> s1tokens, std::vector<std::string>& hyptokens, int mode){
    ...
    std::pair<double, double> ngram(std::vector<std::string>& s1tokens, std::vector<std::string>& hyptokens, int mode){
      //std::cerr<<"ng: " << s1tokens.size()<<std::endl;
      //std::cerr<<"ng: " << hyptokens.size()<<std::endl;
      if (s1tokens.size() != hyptokens.size()){
	std::cerr <<"ERROR: Fail to compute ngram similarity - s1 ngram size != hyp ngram size. Exiting..."<<std::endl;
        exit(1);
      }
      double presult = 0.0;
      double rresult = 0.0;
    ...
    std::pair<double, double> nwpr(std::vector<std::string>& s1tokens, std::vector<std::string>& hyptokens, int mode){
	...
	std::cerr <<"ERROR: Fail to compute ngram precision - ref ngram size != hyp ngram size. Exiting..."<<std::endl;
        exit(1);
      }
