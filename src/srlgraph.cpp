 ...
 * COMMENTS:
 *
 * Class implementation fo the classes:
 *    - srlgraph_t
...
ostream& srlgraph_t::operator<<(ostream& os){
  ...
  if (preds.size() > 0){
	...
	frame_tokens[pred_span.first] = "[" + get_role_label(*it) + " " + frame_tokens[pred_span.first];
	...
	for (vector<srlnid_type>::iterator jt=args.begin(); jt!=args.end(); jt++){
	  ...
	  frame_tokens[arg_span.first] = "[" + get_role_label(*jt) + " " + frame_tokens[arg_span.first];
