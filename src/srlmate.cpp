 ...
 * COMMENTS:
 *
 * Class implementation of srlmate 
 *
 */
...
srlmate_t::srlmate_t(string path){
  ...
  if (!fin){
    cerr<<"ERROR: Fail to open MATE config file ("<<path<<"). Exiting..."<<endl;
    exit(1);
  }
  string matejar="";
  string lang="";
  ...
  JNI_SAFE_CALL(init, jen_m, GetMethodID(mcls, "init", "(Ljava/lang/String;ZZLjava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;"));
  ...
  if (! error.empty()){
    cerr <<"ERROR: Fail to initialize yisi.Mate ("<<error<<"). Exiting..."<<endl;
    //exit(1);
  }
  mate_class_m = mcls;
  mate_object_m = mobj;
...
string srlmate_t::noparse(vector<string> tokens){
  ...
  for (size_t i=0; i<tokens.size(); i++){
    ...
    result+=to_string(i+1)+"\t"+tokens[i]+"\t--\t--\t_\t_\t_\t_\t"+to_string(i)+"\t"+to_string(i)+"\t--\t--\t_\t_\n";
  }
...
string srlmate_t::jrun(string sent){
  ...
  if (! sent.empty() && tokens.size()<=100){
    JNI_SAFE_CALL(methid, jen_m, GetMethodID(mate_class_m, "parse", "(Ljava/lang/String;)Ljava/lang/String;"));
    try {
      JNI_SAFE_CALL(jparse, jen_m, CallObjectMethod(mate_object_m, methid, jen_m->NewStringUTF(sent.c_str())));
