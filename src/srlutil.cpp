 ...
 * COMMENTS:
 *
 * The definitions of some utility classes and functions working on semantic role labeling.
 *
 */
...
void yisi::resolve_arg_span(map<int, vector<int> > child, int curid, srlgraph_t::srlnid_type pid, size_t& b, size_t&e){
...
vector<srlgraph_t> yisi::read_conll09batch(string filename){
  ...
  if (!fin){
    cerr<<"ERROR: Fail to open conll09 parse  file ("<<filename<<"). Exiting..."<<endl;
    exit(1); 
  }
  string parse;
  while (!fin.eof()){
