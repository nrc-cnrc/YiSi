...
struct eval_options {
  ...
  void init(com::masaers::cmdlp::parser& p) {
    ...
    p.add(make_knob(inpidemb_file_m))
      .fallback("")
      .desc("Filename to input subword units with contextual embeddings: one unit per line, empty line separates sentences [unitid<TAB>tokenid<TAB>space_sep_emb].")
      .name("inpidemb-file")
      ;
    p.add(make_knob(hypidemb_file_m))
      .fallback("")
      .desc("Filename to hypotheses subword units with contextual embeddings: one unit per line, empty line separates sentences [unitid<TAB>tokenid<TAB>space_sep_emb].")
      .name("hypidemb-file")
      ;
    p.add(make_knob(refidemb_file_m))
      .fallback("")
      .desc("Filename to reference subword units with contextual embeddings separated by ':': one unit per line, empty line separates sentences [unitid<TAB>tokenid<TAB>space_sep_emb].")
      .name("refidemb-file")
      ;
