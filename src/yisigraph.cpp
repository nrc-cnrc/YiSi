...
yisigraph_t::yisigraph_t(const vector<srlgraph_t> refsrlgraph, const srlgraph_t hypsrlgraph, const srlgraph_t inpsrlgraph){
  refsrlgraph_m = refsrlgraph;
  hypsrlgraph_m = hypsrlgraph;
...
double yisigraph_t::get_sentlength(int mode, int refid){
  ...
  case yisi::REF_MODE:
    if (-1<refid && refid<(int)refsrlgraph_m.size()){
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Sent length undefined. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
double yisigraph_t::get_sentsim(int mode, int refid){
  ...
  case yisi::INP_MODE:
    ...
    break;
  case yisi::HYP_MODE:
    for (auto it=hypalignment_m[hypsrlgraph_m.get_root()].begin(); it!=hypalignment_m[hypsrlgraph_m.get_root()].end(); it++){
  ...
  case yisi::REF_MODE:
    if (-1<refid && refid<(int)refsrlgraph_m.size()){
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Sent sim undefined. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
vector<yisigraph_t::srlnid_type> yisigraph_t::get_preds(int mode, int refid){
  ...
  case yisi::INP_MODE:
    if (inp_b){
      ...
      cerr <<"ERROR: YiSi graph with no input sentence, fail to get input predicates. Exiting..." <<endl;
      exit(1);
    }
    break;
  case yisi::HYP_MODE:
  ...
  case yisi::REF_MODE:
    if (-1<refid && refid<(int)refsrlgraph_m.size()){
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Fail to get reference predicates. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
vector<yisigraph_t::srlnid_type> yisigraph_t::get_args(srlnid_type roleid, int mode, int refid){
  ...
  case yisi::INP_MODE:
    if (inp_b){
      ...
      cerr <<"ERROR: YiSi graph with no input sentence, fail to get input roles. Exiting..." <<endl;
      exit(1);
    }
    break;
  case yisi::HYP_MODE:
  ...
  case yisi::REF_MODE:
    if (-1<refid && refid<(int)refsrlgraph_m.size()){
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Fail to get reference roles. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
vector<string>& yisigraph_t::get_sentence(int mode, int refid){
  ...
  case yisi::INP_MODE:
    if (inp_b){
      ...
      cerr <<"ERROR: YiSi graph with no input sentence, fail to get input sentence. Exiting..." <<endl;
      exit(1);
    }
    break;
  case yisi::HYP_MODE:
  ...
  case yisi::REF_MODE:
    if (-1<refid && refid<(int)refsrlgraph_m.size()){
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Fail to get reference sentence. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
vector<string> yisigraph_t::get_role_fillers(srlnid_type roleid, int mode, int refid){
  ...
  case yisi::INP_MODE:
    if (inp_b){
      ...
      cerr <<"ERROR: YiSi graph with no input sentence, fail to get input role fillers. Exiting..." <<endl;
      exit(1);
    }
    break;
  case yisi::HYP_MODE:
  ...
  case yisi::REF_MODE:
    if (-1<refid && refid<(int)refsrlgraph_m.size()){
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Fail to get reference role fillers. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
double yisigraph_t::get_rolespanlength(srlnid_type roleid, int mode, int refid){
  ...
  case yisi::INP_MODE:
    if (inp_b){
      ...
      cerr <<"ERROR: YiSi graph with no input sentence, fail to get input role span length. Exiting..." <<endl;
      exit(1);
    }
    break;
  case yisi::HYP_MODE:
  ...
  case yisi::REF_MODE:
    if (-1<refid && refid<(int)refsrlgraph_m.size()){
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Fail to get reference role span length. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
yisigraph_t::label_type yisigraph_t::get_rolelabel(srlnid_type roleid, int mode, int refid){
  ...
  case yisi::INP_MODE:
    if (inp_b){
      ...
      cerr <<"ERROR: YiSi graph with no input sentence, fail to get input role label. Exiting..." <<endl;
      exit(1);
    }
    break;
  case yisi::HYP_MODE:
  ...
  case yisi::REF_MODE:
    if (-1<refid && refid<(int)refsrlgraph_m.size()){
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Fail to get reference role label. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
double yisigraph_t::get_alignsim(srlnid_type roleid, int mode, int refid){
  ...
  case yisi::INP_MODE:
	...
	return 0.0;
      ...
      cerr <<"ERROR: YiSi graph with no input sentence, fail to get input alignment sim. Exiting..." <<endl;
      exit(1);
    }
    break;
  case yisi::HYP_MODE:
    ...
    if (hypalignment_m.find(roleid)!=hypalignment_m.end()){
      for (auto it=hypalignment_m[roleid].begin(); it!=hypalignment_m[roleid].end(); it++){
	double s = (it->second).second;
	if (s>result){
	  result=s;
	}
  ...
  case yisi::REF_MODE:
	...
	return 0.0;
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Fail to get reference alignment sim. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
...
yisigraph_t::label_type yisigraph_t::get_alignlabel(srlnid_type roleid, int mode, int refid){
  ...
  case yisi::INP_MODE:
	...
	return label_type("U");
      ...
      cerr <<"ERROR: YiSi graph with no input sentence, fail to get input alignment role label. Exiting..." <<endl;
      exit(1);
    }
    break;
  case yisi::HYP_MODE: 
  ...
  case yisi::REF_MODE:
	...
	return label_type("U");
      ...
      cerr <<"ERROR: refid (" <<refid <<") out of range [0," <<refsrlgraph_m.size() <<"]. Fail to get reference alignment role label. Exiting..." <<endl;
      exit(1);
    }
    break;
  default:
