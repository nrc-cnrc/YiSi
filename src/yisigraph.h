 ...
 *    - yisigraph_t
 * and the function defintion of yisi alignment algorithm.
 *
 */
  ...
  void yisigraph_t::align(phrasesim_t<T>* phrasesim){
	...
	if (refpredspan.first != refpredspan.second){
	  ...
	  for (auto jt=hyppreds.begin(); jt!=hyppreds.end(); jt++){
	    ...
	    if (hyppredspan.first != hyppredspan.second){
	      ...
	      std::pair<double, double> predsim = (*phrasesim)(refpredphrase, hyppredphrase, yisi::REF_MODE);
	...
	for (auto it=refargs.begin(); it!=refargs.end(); it++){
	  ...
	  for (auto jt=hypargs.begin(); jt!=hypargs.end(); jt++){
	    ...
	    std::pair<double, double> argsim = (*phrasesim)(refargphrase, hypargphrase, yisi::REF_MODE);
	...
	hypalignment_m[aligned_hyp_pred].push_back(std::make_pair(refid, alignment_type(aligned_ref_pred, psim)));
	auto refargs = refsrlgraph_m[refid].get_args(aligned_ref_pred);
	auto hypargs = hypsrlgraph_m.get_args(aligned_hyp_pred); 
	maxmatching_t argmatch;
	for (auto it=refargs.begin(); it!=refargs.end(); it++){
	...
	for (size_t j=0; j<ar.size(); j++){
	  ...
	  if (hypalignment_m.find(aligned_hyp_arg) == hypalignment_m.end()){
	    hypalignment_m[aligned_hyp_arg]=std::vector<std::pair<int, alignment_type> >();
	  }
	  hypalignment_m[aligned_hyp_arg].push_back(std::make_pair(refid, alignment_type(aligned_ref_arg, asim)));
	}
      }
    }
    //input
	...
	hypalignment_m[hyproot]=std::vector<std::pair<int, alignment_type> >();
      ...
      hypalignment_m[hyproot].push_back(std::make_pair((int)refsrlgraph_m.size(), alignment_type(inproot, sentsim.first)));
	...
	if (inppredspan.first != inppredspan.second){
	  ...
	  for (auto jt=hyppreds.begin(); jt!=hyppreds.end(); jt++){
	    ...
	    if (hyppredspan.first != hyppredspan.second){
	      auto hyppredphrase = hypsrlgraph_m.get_role_fillers(hyppredid);
	      std::pair<double, double> predsim = (*phrasesim)(inppredphrase, hyppredphrase, yisi::INP_MODE);
	...
	for (auto it=inpargs.begin(); it!=inpargs.end(); it++){
	  ...
	  for (auto jt=hypargs.begin(); jt!=hypargs.end(); jt++){
	    auto hypargid = *jt;
	    auto hypargphrase = hypsrlgraph_m.get_role_fillers(hypargid);
	    std::pair<double, double> argsim = (*phrasesim)(inpargphrase, hypargphrase, yisi::INP_MODE);
	...
	hypalignment_m[aligned_hyp_pred].push_back(std::make_pair((int)refsrlgraph_m.size(), alignment_type(aligned_inp_pred, psim)));
	auto inpargs = inpsrlgraph_m.get_args(aligned_inp_pred);
	auto hypargs = hypsrlgraph_m.get_args(aligned_hyp_pred);
	maxmatching_t argmatch;
	for (auto it=inpargs.begin(); it!=inpargs.end(); it++){
	...
	for (size_t j=0; j<ar.size(); j++){
	  ...
	  hypalignment_m[aligned_hyp_arg].push_back(std::make_pair((int)refsrlgraph_m.size(), alignment_type(aligned_inp_arg, asim)));
	}
      }
    }
  }
