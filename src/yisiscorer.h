...
namespace yisi {
  ...
  public:
	...
	if (!LBL){
	  std::cerr <<"ERROR: fail to open labelconfig. Exiting..."<<std::endl;
	  exit(1);
	}
	...
	std::cerr <<"Done."<<std::endl;
      ...
      if (weightconfig_path_m != "" && weightconfig_path_m != "lexweight" && weightconfig_path_m != "uniform"){
	std::ifstream W(weightconfig_path_m.c_str());
	if (!W){
	  std::cerr <<"ERROR: fail to open weightconfig. Exiting..."<<std::endl;
	  exit(1);
	}
	...
	if ((int)weight_m.size() != i){
	  std::cerr <<"ERROR: number of weights in weightconfig does not match with number of lines in labelconfig. Exiting..."<<std::endl;
	  exit(1);
	}
      } else {
	for (int j=0; j<i; j++){
      ...
      outsrl_p = new srl_t(opt.outsrl_name_m, opt.outsrl_path_m);
      outsrl_name_m = opt.outsrl_name_m;
	...
	for (auto jt=preds.begin(); jt!=preds.end(); jt++){
	  ...
	  if (label_m.find(pred_label)==label_m.end()){
	    std::cerr<<"ERROR: unknown srl label '"<<pred_label<<"'. Check your labelconfig. Exiting..."<<std::endl;
	    exit(1);
	  }
	  ...
	  for (auto kt=args.begin(); kt!=args.end(); kt++){
	    ...
	    if (label_m.find(arg_label)==label_m.end()){
	      std::cerr<<"ERROR: unknown srl label '"<<arg_label<<"'. Check your labelconfig. Exiting..."<<std::endl;
	      exit(1);
	    }
    ...
    std::vector<srlgraph_t> refsrlparse(std::vector<std::string> refsents){
      ...
      std::vector<srlgraph_t> result = outsrl_p->parse(refsents);
      //std::cerr<<"Done."<<std::endl;
      if (weightconfig_path_m == ""){
	this->estimate_weight(result);
      }
    ...
    std::vector<srlgraph_t> hypsrlparse(std::vector<std::string> hypsents){
      ...
      std::vector<srlgraph_t> result = outsrl_p->parse(hypsents);
      //std::cerr<<"Done."<<std::endl;
      return result;
    };
    srlgraph_t hypsrlparse(std::string hypsent){
      //std::cerr<<"Tokenizing/SRL-ing the hypothesis ... ";
      srlgraph_t result = outsrl_p->parse(hypsent);
      //std::cerr<<"Done."<<std::endl;
      return result;
    };
    ...
    yisigraph_t align(const std::vector<srlgraph_t> refsrlgraph, const srlgraph_t hypsrlgraph, const srlgraph_t inpsrlgraph){
	...
	if (precision == 0.0 || recall == 0.0){
	  ...
	  if (prfunc_name_m == "f" || prfunc_name_m == "lexexp"){
	    ...
	    std::cerr<<"ERROR: unknown precision/recall agg function name. Exiting ..."<<std::endl;
	    exit(1);
	  }
	}
	if (mode_m == "yisi" || mode_m == "yisi_flat" || mode_m == "features"){
	  return yisi;
  ...
  private:
	...
	if (sanity_check > 0){
	  // if (prfunc_name_m=="f" || prfunc_name_m=="max"){
	    ...
	    for (auto jt=args.begin(); jt!=args.end(); jt++){
	      ...
	      if (argsim >= rolesim_threshold_m && match(arglabel, alignlabel)){
		fn += argweight * argsim;
	      }
	      fd += argweight;
	    }
    ...
    void compute_features(yisigraph_t yg, std::vector<double> feats, double& structure, double& flat, int mode, int refid=-1){
	...
	if (label_m.find(label1)==label_m.end()){
	  std::cerr<<"ERROR: unknown srl label '"<<label1<<"' in yisi for matching label 1. Check you labelconfig. Exiting..."<<std::endl;
	  exit(1);
	}
	if (label_m.find(label2)==label_m.end()){
	  std::cerr<<"ERROR: unknown srl label '"<<label2<<"' in yisi for matching label 2. Check you labelconfig. Exiting..."<<std::endl;
	  exit(1);
	}
	...
	if (label_m.find(label) == label_m.end()){
	  std::cerr<<"ERROR: unknown srl label '"<<label<<"' in yisi for get_weight. Check you labelconfig. Exiting..."<<std::endl;
	  exit(1);
	}
